# DevTools Hub

개발자, 디자이너, 마케터를 위한 온라인 유틸리티 도구 모음입니다.

## 주요 기능

### 개발자 도구

| 도구 | 설명 | 상태 |
|------|------|------|
| **JSON Formatter** | JSON 데이터를 보기 좋게 포맷팅하고 유효성을 검사합니다 | ✅ |
| **Base64 Encoder** | 텍스트나 데이터를 Base64로 인코딩/디코딩합니다 | ✅ |
| **Code Runner** | JavaScript, Python 등 다양한 언어의 코드를 실행합니다 | ✅ |
| **Regex Tester** | 정규표현식을 테스트하고 매칭 결과를 확인합니다 | ✅ |
| **Markdown Editor** | 마크다운 편집 및 실시간 미리보기 | ✅ |

### 디자이너 도구

| 도구 | 설명 | 상태 |
|------|------|------|
| **Color Picker** | 다양한 형식(HEX, RGB, HSL)으로 색상을 선택하고 변환합니다 | ✅ |
| **Color Palette** | 조화로운 색상 팔레트를 생성하고 관리합니다 | ✅ |
| **Gradient Generator** | CSS 그라디언트를 시각적으로 생성합니다 | ✅ |
| **Image Editor** | 이미지 크기 조절, 자르기, 필터 적용 | ✅ |

### 마케터 도구

| 도구 | 설명 | 상태 |
|------|------|------|
| **QR Code Generator** | QR 코드를 생성하고 다운로드합니다 | ✅ |
| Link Shortener | URL 단축 | 🚧 |

### 생산성 도구

| 도구 | 설명 | 상태 |
|------|------|------|
| **Password Generator** | 안전한 비밀번호를 생성합니다 | ✅ |
| Unit Converter | 단위 변환 | 🚧 |
| Timezone Converter | 시간대 변환 | 🚧 |
| Word Counter | 글자 수, 단어 수 분석 | 🚧 |
| Lorem Ipsum Generator | 더미 텍스트 생성 | 🚧 |

## 기술 스택

### Core
- **Next.js 16** - React 프레임워크
- **React 19** - UI 라이브러리
- **TypeScript 5** - 타입 안전성

### 스타일링
- **Tailwind CSS 4** - 유틸리티 CSS
- **Radix UI** - 접근성 컴포넌트
- **Lucide React** - 아이콘
- **next-themes** - 다크모드 지원

### 상태 관리 & 폼
- **Zustand** - 클라이언트 상태 관리
- **TanStack Query** - 서버 상태 관리
- **React Hook Form + Zod** - 폼 검증

### 도구 특화
- **Monaco Editor** - 코드 에디터 (Code Runner)
- **Pyodide** - 브라우저에서 Python 실행
- **Colord** - 색상 변환 라이브러리
- **marked** - 마크다운 파싱
- **highlight.js** - 코드 하이라이팅
- **qrcode.react** - QR 코드 생성
- **html2canvas-pro** - 스크린샷 캡처
- **react-colorful** - 색상 선택기

## 프로젝트 구조

```
src/
├── app/                        # Next.js App Router
│   ├── (main)/                 # 메인 도구 페이지들
│   │   ├── base64-encoder/
│   │   ├── code-runner/
│   │   ├── color-palette/
│   │   ├── color-picker/
│   │   ├── gradient-generator/
│   │   ├── image-editor/
│   │   ├── json-formatter/
│   │   ├── markdown-editor/
│   │   ├── password-generator/
│   │   ├── qr-generator/
│   │   └── regex-tester/
│   ├── admin/                  # 관리자 페이지
│   │   ├── login/
│   │   └── dashboard/
│   └── providers/              # 컨텍스트 프로바이더
├── features/                   # 기능별 모듈
│   └── tools/                  # 도구별 비즈니스 로직
│       ├── base64-encoder/
│       ├── code-runner/
│       ├── color-palette/
│       ├── color-picker/
│       ├── gradient-generator/
│       ├── image-editor/
│       ├── json-formatter/
│       ├── markdown-editor/
│       ├── password-generator/
│       ├── qr-generator/
│       └── regex-tester/
├── shared/                     # 공용 유틸리티 & UI
│   ├── api/                    # API 클라이언트
│   ├── config/                 # 설정
│   ├── hooks/                  # 커스텀 훅
│   ├── lib/                    # 유틸리티 함수
│   └── ui/                     # 공용 UI 컴포넌트
└── widgets/                    # 페이지 구성 위젯
    ├── header/
    ├── footer/
    ├── sidebar/
    ├── hero-section/
    ├── tools-grid/
    ├── tool-card/
    ├── cta-section/
    ├── developer-section/
    └── stats-section/
```

> Feature-Sliced Design (FSD) 아키텍처를 따릅니다.

## 시작하기

### 요구 사항
- Node.js 18+
- npm 또는 yarn

### 설치

```bash
# 저장소 클론
git clone <repository-url>
cd toolbox

# 의존성 설치
npm install
```

### 개발 서버 실행

```bash
npm run dev
```

[http://localhost:3000](http://localhost:3000)에서 확인할 수 있습니다.

### 프로덕션 빌드

```bash
npm run build
npm run start
```

## 스크립트

| 명령어 | 설명 |
|--------|------|
| `npm run dev` | 개발 서버 실행 |
| `npm run build` | 프로덕션 빌드 |
| `npm run start` | 프로덕션 서버 실행 |
| `npm run lint` | ESLint 검사 |

## 라이선스

Private
